# model1_disease_detection_full.py
...
"Tungro": [
        # os.path.join(hduytrng_mendeley_rice_disease_dataset_path, "Augmented Images", "Leaf Blast"),
        os.path.join(nirmalsankalana_rice_leaf_disease_image_path, "Tungro"),
        # os.path.join(dedeikhsandwisaputra_rice_leafs_disease_dataset_path, "RiceLeafsDisease", "train", "leaf_blast"),
    ],
    "Healthy": [
...
EPOCHS = 120# Number of times to train on the full dataset
...
USE_GAN_AUG = True  # Set True to enable GAN-based augmentation (heavy)
GAN_EPOCHS = 10     # Keep low for experimentation; increase for better quality
...
# Plot Combined Training & Validation Accuracy-Loss Curve (single plot)
...
plt.legend()
plt.tight_layout()
acc_loss_curve_path = os.path.join(OUTPUT_DIR, "acc_loss_curve.png")
plt.savefig(acc_loss_curve_path)
plt.close()

# Plot Combined Training & Validation Accuracy (initial + fine-tuning)
plt.figure(figsize=(8, 6))
combined_train_acc = history.history['sparse_categorical_accuracy'] + history_fine.history['sparse_categorical_accuracy']
combined_val_acc = history.history['val_sparse_categorical_accuracy'] + history_fine.history['val_sparse_categorical_accuracy']
plt.plot(combined_train_acc, label='Train Accuracy (Combined)')
plt.plot(combined_val_acc, label='Validation Accuracy (Combined)')
plt.title("Training & Validation Accuracy (Pre- and Fine-Tuning)")
plt.xlabel("Epochs")
plt.ylabel("Accuracy")
plt.legend()
...
# c.drawString(50, current_y, "Rice Leaf Disease Detection - Model Results Summary")
# current_y -= 30
# PDF helper to add multi-line wrapped text
def add_text(c, text_lines, x, y, line_height=14, font="Courier", font_size=10, max_width=480):
    ...
    for line in text_lines:
        if stringWidth(line, font, font_size) > max_width:
            ...
            buffer = ""
            for word in words:
                test_line = buffer + ' ' + word if buffer else word
                if stringWidth(test_line, font, font_size) <= max_width:
                    buffer = test_line
                else:
                    c.drawString(x, current_y, buffer)
...
# Create PDF and setup variables

pdf_path = os.path.join(OUTPUT_DIR, "model_results_summary.pdf")
...
c.drawString(margin_x, current_y, "Rice Leaf Disease Detection - Model Results Summary")
current_y -= 40

c.setFont("Helvetica", 12)
c.drawString(margin_x, current_y, f"Number of classes: {len(CLASS_NAMES)}")
current_y -= 20
c.drawString(margin_x, current_y, f"Classes: {', '.join(CLASS_NAMES)}")
current_y -= 40

# Plot 1: Accuracy Curve (initial training)
c.setFont("Helvetica-Bold", 14)
c.drawString(margin_x, current_y, "Training Accuracy and Validation Accuracy (Initial Training)")
current_y -= 10
acc_img = ImageReader(accuracy_path)
c.drawImage(acc_img, margin_x, current_y - 220, width=500, height=220, preserveAspectRatio=True, mask='auto')
current_y -= 240

# Plot 2: Loss Curve (initial training)
c.setFont("Helvetica-Bold", 14)
c.drawString(margin_x, current_y, "Training Loss and Validation Loss (Initial Training)")
current_y -= 10
loss_img = ImageReader(loss_path)
c.drawImage(loss_img, margin_x, current_y - 220, width=500, height=220, preserveAspectRatio=True, mask='auto')
current_y -= 240
...
# Plot 3: Combined Accuracy/Loss curve
c.setFont("Helvetica-Bold", 14)
c.drawString(margin_x, current_y, "Combined Training & Validation Accuracy / Loss")
current_y -= 10
acc_loss_img = ImageReader(acc_loss_curve_path)
c.drawImage(acc_loss_img, margin_x, current_y - 280, width=500, height=280, preserveAspectRatio=True, mask='auto')
current_y -= 290

c.showPage()  # New page

# Plot 4: Combined Train & Val Accuracy (Pre- + Fine-Tuning)
current_y = height - 50
c.setFont("Helvetica-Bold", 14)
c.drawString(margin_x, current_y, "Combined Training & Validation Accuracy (Pre- and Fine-Tuning)")
current_y -= 10
train_curve_img = ImageReader(training_curve_path)
c.drawImage(train_curve_img, margin_x, current_y - 280, width=500, height=280, preserveAspectRatio=True, mask='auto')
current_y -= 290
...
current_y = height - 50
c.setFont("Helvetica-Bold", 16)
c.drawString(margin_x, current_y, "Confusion Matrix")
current_y -= 30
cm_img = ImageReader(cm_path)
c.drawImage(cm_img, margin_x, current_y - 300, width=500, height=300, preserveAspectRatio=True, mask='auto')
current_y -= 320
...
current_y = height - 50
c.setFont("Helvetica-Bold", 16)
c.drawString(margin_x, current_y, "Classification Report (Precision, Recall, F1-Score)")
current_y -= 30
report_lines = report_text.strip().split('\n')
current_y = add_text(c, report_lines, margin_x, current_y, line_height=14, font="Courier", font_size=10)
...
current_y = height - 50
c.setFont("Helvetica-Bold", 16)
c.drawString(margin_x, current_y, "AUC (Area Under ROC Curve) per Class")
current_y -= 30
auc_lines = [f"{cls}: {auc:.4f}" if not np.isnan(auc) else f"{cls}: N/A" for cls, auc in aucs.items()]
current_y = add_text(c, auc_lines, margin_x, current_y, line_height=14, font="Courier", font_size=12)

c.showPage()  # Last page
...
current_y -= 30
notes = [
    "This report summarizes the training, validation, and evaluation",
    "of the Rice Leaf Disease Detection model.",
    "Key metrics including accuracy, loss, confusion matrix,",
    "classification report, and AUC per class are displayed.",
    "Graphs are generated from training history and test evaluation.",
    "The model architecture uses ResNet50 with attention and multi-scale blocks.",
    "GAN-based augmentation was optionally used for robustness.",
    "Grad-CAM visualizations and TFLite quantized model are also available.",
]
current_y = add_text(c, notes, margin_x, current_y, line_height=14, font="Helvetica", font_size=11)
...
last_conv_layer = "conv2d_12"  # Example, replace with correct layer
